...
HOURLY_DIR = os.path.join(BASE_DIR, "data", "hourly")
...
def fetch_data(instrument, to_ts):
    """Fetch historical hourly data from CoinDesk"""
    params = {
        "market": "cadli",
...
# =============================
# COLLECT 3 YEARS (CHUNKED)
# =============================
def get_all_hourly_data(instrument):
    """Collect 3 years of hourly data"""
    all_data = []
    current_end = END_DATE

    while current_end > START_DATE:
        print(f"[{instrument}] Fetching up to {current_end}")
        ...
        # Move backward using oldest timestamp in chunk
        oldest_ts = min(row["TIMESTAMP"] for row in chunk)
        current_end = datetime.datetime.utcfromtimestamp(oldest_ts) - datetime.timedelta(hours=1)
...
def clean_data(data):
    ...
    return cleaned[::-1]  # Reverse for oldest first
...
if __name__ == "__main__":
    print(f"\n📥 Downloading {len(COINS)} cryptocurrencies (hourly data)...\n")

    for coin in COINS:
        print(f"\n🔄 Processing {coin}...")
        data = get_all_hourly_data(coin)
        cleaned = clean_data(data)
        save_csv(cleaned, f"{coin}-INR_hourly.csv")

    print("\n✅ All hourly data downloaded successfully!")
